...
class PT_TW_DDModel:
    def __init__(self):
        ...
        self.all_errors = {}  # Add this to store errors for all subjects
    ...
    def expected_value_gain(self, day: int, price: int, amount: int, fit: Parameters) -> float:
        ...
        ev = 0.0
        max_price = 15
        for future_day in range(day + 1, 69):
    ...
    def predict_sales(self, subject: int, days: np.ndarray, stored: np.ndarray, prices: np.ndarray, actual_sold: np.ndarray, params: Parameters) -> np.ndarray:
        ...
        for idx, day in enumerate(days):
            if self.verbose:  # Only print if verbose is True
                print(f"Processing Day {day} for Subject {subject}")
            amount = stored[idx]
            price = prices[idx]
    ...
    def fit_one_subject(self, subject: int, start_fit: Parameters) -> None:
        ...
        try:
            ...
            result = basinhopping(
                objective_function,
                x0,
                niter=1000,          # Increased from 100 to 500 iterations
        ...
        # Print MSE from error_of_fit
            final_mse = self.error_of_fit(subject, best_fit)
            print(f"\nFinal Mean Squared Error for Subject {subject}: {final_mse:.4f}")
    ...
    def fit_all_subjects(self, start_fit: Parameters):
        """Fit the model for all subjects and save error metrics."""
        all_errors = []
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_file = os.path.join(DATA_DIR, f'model_errors_{timestamp}.txt')

        with open(output_file, 'w') as f:
            f.write("PT-TW-DD Model Error Report\n")
            ...
            for subject in self.data['Subject'].unique():
                ...
                if subject in self.best_fits:
                    error = self.error_of_fit(subject, self.best_fits[subject])
                    self.all_errors[subject] = error
                    all_errors.append(error)
                    f.write(f"Subject {subject}: MSE = {error:.4f}\n")
                else:
                    f.write(f"Subject {subject}: Failed to fit\n")
            ...
            f.write("\nSummary Statistics:\n")
            f.write("-" * 50 + "\n")
            f.write(f"Mean MSE: {mean_error:.4f}\n")
            f.write(f"Std MSE: {std_error:.4f}\n")
            f.write(f"Min MSE: {min(all_errors):.4f}\n")
            f.write(f"Max MSE: {max(all_errors):.4f}\n")

        print(f"\nError metrics have been saved to: {output_file}")
        return mean_error
...
def main(version: str):
    ...
    print(f"\nOverall mean error across all subjects: {mean_error:.4f}")
