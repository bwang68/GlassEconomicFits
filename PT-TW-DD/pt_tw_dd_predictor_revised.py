...
class PT_TW_DD_Model:
    ...
    def PTv3(self, day: int, price: int, sold: int, params: Parameters, c_o: List[int]) -> float:
        ...
        # Calculate epsilon denominator once
        epsilon_sum = sum(1 + params.epsilon * (k - 1) for k in range(1, c_o[t-1]))
...
def fit_all_participants(model: PT_TW_DD_Model) -> dict:
